...
def generate_executive_summary(project_files, task_id=None, convert_pdf=True):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if convert_pdf:
                ...
                print("¡Auditoría IA completada con éxito!")
                update_progress(task_id, 40, "IA: ¡Auditoría construida! Pasando al ensamblador Win32...")
                return {"docx": output_ai_filename_docx}
        else:
...
def compile_unified_word_v2(master_template_path, project_files, output_path, font_family="default", export_format="word", task_id=None,
                            general_font_size=11, title_font_size=16, subtitle_font_size=13, table_font_size=10, caption_font_size=9):
    try: general_font_size = float(general_font_size)
    except: general_font_size = 11.0
    try: title_font_size = float(title_font_size)
    except: title_font_size = 16.0
    try: subtitle_font_size = float(subtitle_font_size)
    except: subtitle_font_size = 13.0
    try: table_font_size = float(table_font_size)
    except: table_font_size = 10.0
    try: caption_font_size = float(caption_font_size)
    except: caption_font_size = 9.0
    ...
    try:
        ...
        try:
            ...
            for para in doc.Paragraphs:
                try:
                    ...
                    # Detect table caption paragraphs (starts with 'tabla')
                    if re.match(r'^\s*(?:[\d\.]+\s*)?tabla\b', text, re.IGNORECASE):
                        para_pos = para.Range.Start
                        current_prefix = get_prod_id_for_pos(para_pos)

                        # Check for existing numbering e.g., "1.1.2 Tabla" or "1.1 Tabla"
                        existing_match = re.match(r'^\s*(\d+\.\d+)(?:\.(\d+))?\s+tabla', text, re.IGNORECASE)
                        if existing_match:
                            # Use existing sequence if present, else start at 1
                            seq = int(existing_match.group(2)) if existing_match.group(2) else 1
                        else:
                            # Increment sequence for this product prefix
                            seq = table_seq_by_prefix.get(current_prefix, 0) + 1

                        # Update the tracker for the current prefix
                        if current_prefix:
                            table_seq_by_prefix[current_prefix] = max(seq, table_seq_by_prefix.get(current_prefix, 0))
                            new_text = f"{current_prefix}.{seq} Tabla"
                        else:
                            new_text = "Tabla"

                        # Apply replacement in the document
                        rng = para.Range
        ...
        try:
            ...
            if toc_entries:
                ...
                # OPTIMIZED: Linear enumeration scan instead of quadratic index-based Paragraphs(i)
                for para_idx, para in enumerate(doc.Paragraphs, 1):
                    try:
                        p_text = para.Range.Text.strip()
                        if re.search(r'tabla\s+de\s+contenido', p_text, re.IGNORECASE):
                            toc_insert_para_idx = para_idx
                            toc_placeholder_range = para.Range
                            break
                    except:
                        pass

                font_name = "Arial" if font_family == "default" else font_family
                ...
                if toc_placeholder_range:
                    ...
                    toc_placeholder_range.Font.Name = font_name
                    ...
                    # Now format the newly inserted paragraphs using fast .Next traversal instead of slow index lookups
                    try:
                        curr_para = toc_placeholder_range.Paragraphs(1)
                        curr_para = curr_para.Next # Blank separator line
                        for level, _ in toc_lines:
                            curr_para = curr_para.Next
                            if not curr_para:
                                break
                            curr_para.Range.Font.Name = font_name
                            curr_para.Range.Font.Bold = (level == 1)
                            curr_para.Range.Font.Size = 11 if level == 1 else 10
                            curr_para.Range.ParagraphFormat.Alignment = 0
                    except Exception as e_toc_format:
                        print("Error formatting custom TOC paragraphs natively:", e_toc_format)
                    ...
                    first_para.Range.Font.Name = font_name
                    first_para.Range.Font.Size = 16
                    first_para.Range.Font.Bold = True
                    first_para.Range.ParagraphFormat.Alignment = 1

                    # Style prepended TOC entries using fast .Next traversal
                    try:
                        curr_para = first_para
                        curr_para = curr_para.Next # Blank separator line
                        for level, _ in toc_lines:
                            curr_para = curr_para.Next
                            if not curr_para:
                            ...
                            curr_para.Range.Font.Size = 11 if level == 1 else 10
                            curr_para.Range.ParagraphFormat.Alignment = 0
                    except Exception as e_fallback_format:
                        print("Error formatting fallback TOC paragraphs natively:", e_fallback_format)
        ...
        # ===== STEP 6: Custom Font Sizing Styling Pass =====
        update_progress(task_id, 92, "Win32 V2: Aplicando tamaños de fuente personalizados...")
        try:
            # 1. Format tables (fast, single-call Range Font Size styling per table)
            for table in doc.Tables:
                try:
                    table.Range.Font.Size = table_font_size
                except Exception as e_table:
                    pass

            # 2. Format paragraphs (fast direct native iterator)
            for para in doc.Paragraphs:
                try:
                    # Skip if inside table cell (already styled by the table range)
                    ...
                    # Determine if it's a caption, a title, a subtitle or general text
                    if "caption" in style_name or "leyenda" in style_name or re.match(r'^\s*(?:[\d\.]+\s*)?tabla\b', text, re.IGNORECASE):
                        para.Range.Font.Size = caption_font_size
                    elif any(h in style_name for h in ["heading 1", "heading 2", "título 1", "título 2", "titulo 1", "titulo 2"]):
                        para.Range.Font.Size = title_font_size
                    elif any(h in style_name for h in ["heading", "título", "titulo"]):
                        # Any other heading styles (Heading 3, 4 etc.) are subtitles
                        para.Range.Font.Size = subtitle_font_size
                    else:
                        para.Range.Font.Size = general_font_size
                except Exception as e_para:
                    pass
        except Exception as e_font:
            print(f"Error in custom font sizing styling pass: {e_font}")
...
def generate_predictive_report(project_files, task_id=None, convert_pdf=True):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if convert_pdf:
                ...
                print("¡Informe Predictivo IA completado!")
                update_progress(task_id, 38, "IA Predictiva: ¡Informe construido!")
                return {"docx": output_filename_docx}
        else:
...
def worker_compilation_v2(task_id, master_path, project_files, output_path, font_family, generate_ai, mode, export_format,
    ...
    try:
        ...
        if generate_ai:
            update_progress(task_id, 5, "IA: Iniciando análisis inteligente y predictivo en paralelo...")
            
            # Call OpenRouter concurrently to save API call times
            def run_exec():
                nonlocal ai_filenames
                try:
                    ai_filenames = generate_executive_summary(project_files, task_id, convert_pdf=False)
                except Exception as e:
                    print(f"Error thread exec summary: {e}")
                    
            def run_pred():
                nonlocal ai_predictive_filenames
                try:
                    ai_predictive_filenames = generate_predictive_report(project_files, task_id, convert_pdf=False)
                except Exception as e:
                    print(f"Error thread predictive report: {e}")
                    
            t_exec = threading.Thread(target=run_exec)
            t_pred = threading.Thread(target=run_pred)
            ...
            update_progress(task_id, 40, "IA: Análisis paralelo de texto completado. Generando PDFs...")
            
            # Now convert to PDF sequentially using a SINGLE Word instance (prevents COM conflicts)
            try:
                pythoncom.CoInitialize()
                word_conv = win32com.client.DispatchEx("Word.Application")
                word_conv.Visible = False
                word_conv.ScreenUpdating = False
                word_conv.DisplayAlerts = False
                
                if ai_filenames and "docx" in ai_filenames:
                    docx_p = os.path.join(OUTPUT_FOLDER, ai_filenames["docx"])
                    pdf_p = docx_p.replace(".docx", ".pdf")
                    try:
                        doc = word_conv.Documents.Open(docx_p)
                        doc.SaveAs(pdf_p, FileFormat=17)
                        doc.Close()
                        ai_filenames["pdf"] = os.path.basename(pdf_p)
                        print("Executive Summary PDF converted sequentially successfully.")
                    except Exception as e:
                        print(f"Error converting Exec Summary PDF: {e}")
                        
                if ai_predictive_filenames and "docx" in ai_predictive_filenames:
                    docx_p = os.path.join(OUTPUT_FOLDER, ai_predictive_filenames["docx"])
                    pdf_p = docx_p.replace(".docx", ".pdf")
                    try:
                        doc = word_conv.Documents.Open(docx_p)
                        doc.SaveAs(pdf_p, FileFormat=17)
                        doc.Close()
                        ai_predictive_filenames["pdf"] = os.path.basename(pdf_p)
                        print("Predictive Report PDF converted sequentially successfully.")
                    except Exception as e:
                        print(f"Error converting Predictive Report PDF: {e}")
                        
                word_conv.Quit()
            except Exception as e_conv:
                print(f"Error in batch sequential PDF conversion: {e_conv}")
            finally:
                try: pythoncom.CoUninitialize()
                except: pass

        saved_files = compile_unified_word_v2(master_path, project_files, output_path, font_family, export_format, task_id,
                                              general_font_size, title_font_size, subtitle_font_size, table_font_size, caption_font_size)
...
def generate_report_v2():
    ...
    try:
        ...
        generate_ai = request.form.get("generateAI") == "true"
        export_format = request.form.get("exportFormat", "word")
        general_font_size = request.form.get("generalFontSize", "11")
        title_font_size = request.form.get("titleFontSize", "16")
        subtitle_font_size = request.form.get("subtitleFontSize", "13")
        table_font_size = request.form.get("tableFontSize", "10")
        caption_font_size = request.form.get("captionFontSize", "9")

        output_filename = f"Reporte_Mensual_Generado_V2_{task_id[:8]}.docx"
        output_path = os.path.join(OUTPUT_FOLDER, output_filename)
        
        thread = threading.Thread(
            target=worker_compilation_v2, 
            args=(
                task_id, master_path, project_files, output_path, font_family, generate_ai, mode, export_format,
                general_font_size, title_font_size, subtitle_font_size, table_font_size, caption_font_size
            )
        )
        thread.start()
